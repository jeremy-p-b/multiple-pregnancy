...
gformula_formula) {
#   # prepare data
#   complete_twins <- bind_rows(plevel_data %>% filter(no_infants==2) %>% mutate(outcome = as.integer(no_outcomes > 0)), plevel_data %>% filter(no_infants==2) %>% mutate(outcome = as.integer(no_outcomes > 1)))
#   incomplete_twin <- plevel_data %>% filter(no_infants == 1 & probable_multiple == 1)
#   singleton <- plevel_data %>% filter(no_infants == 1 & probable_multiple == 0) %>% mutate(outcome = any_outcome)
#   
#   complete_twins_plevel_out <- plevel_data %>% 
#     filter(no_infants==2 & any_outcome == 1) %>%
#     mutate(both_outcome = as.integer(no_outcomes > 1))
#   
#   #twin_model <- glm(as.formula(glue("both_outcome ~ {exposure}")), family="binomial", data=complete_twins_plevel_out)
#   #preds <- predict(twin_model, newdata=incomplete_twin, type="response")
#   
#   # impute outcome
#   twin_ridge_model <- model.matrix(as.formula(glue("~ 0 + {exposure} + rcs(AGE, best_knots) + delivery_year + obesity + tobacco + alcohol + drug_abuse + diabetes + hypertension + epilepsy + terat + antidep + anticonv + opioids + antibiotic")), 
#                                    data=complete_twins_plevel_out) %>% cv.glmnet(y=complete_twins_plevel_out$both_outcome, family="binomial", alpha=0)
#   
#   preds <- predict(twin_ridge_model, 
#                    newx=model.matrix(as.formula(glue("~ 0 + {exposure} + rcs(AGE, best_knots) + delivery_year + obesity + tobacco + alcohol + drug_abuse + diabetes + hypertension + epilepsy + terat + antidep + anticonv + opioids + antibiotic")), data=incomplete_twin),
#                    type="response", 
#                    s="lambda.min")
#   
#   incomplete_twin[["both"]] <- incomplete_twin[["any_outcome"]] * rbinom(nrow(incomplete_twin),1,preds)
#   imputed_twins <- bind_rows(incomplete_twin, incomplete_twin) %>% group_by(pregid) %>% mutate(outcome = if_else(any_outcome == 1 & row_number() == 2 & both == 0, 0, any_outcome))
#   
#   imputed_data <- bind_rows(complete_twins, imputed_twins, singleton)
#   
#   ddist <- datadist(imputed_data)
#   options("datadist" = "ddist")
#   
#   # fit poisson models
#   imputed_adj_pois_model <- glm(as.formula(glue("outcome ~ {exposure} + rcs(AGE, best_knots) + delivery_year + obesity + tobacco + alcohol + drug_abuse + diabetes + hypertension + epilepsy + terat + antidep + anticonv + opioids + antibiotic")), 
#                                 data=imputed_data, 
#                                 family="poisson")
#   # calculate rd
#   imputed_rd <- calc_risk_estimates(imputed_data, exposure, "outcome", gformula_formula)
#   
#   # combine results
#   imputed_combined <- c(imputed_rd, "conditional adj rr" = exp(imputed_adj_pois_model$coefficients[[exposure]]))
#   return(imputed_combined)
# }
# plevel_data <- analysis_data %>% group_by(pregid) %>% mutate(no_infants=n(), no_outcomes=sum(get(outcome)), any_outcome=max(get(outcome))) %>% filter(row_number() == 1) 
# 
# imputed_estimates <- boot(plevel_data, function(x,y) imputed_analysis(x[y,], exposure, gformula_formula_for_general_outcome), R=1000, parallel="multicore", ncpus=4)
# saveRDS(imputed_estimates, glue("models/imputed_estimates_{outcome}{file_stub}.rds"))
